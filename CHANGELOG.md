...
- Fusion dans `workflow-dev-pur` du contenu survivant d'un ancien document de flux Git (brièvement déposé dans un dossier `rules/` à la racine, jamais chargé par Claude Code, maintenant supprimé). Décisions d'équipe entérinées au passage : nommage de branche, granularité « une PR par issue » et format de titre selon `workflow-dev-pur` ; post-merge = *In QA* ; commits en anglais au format conventionnel (`feat:`/`fix:`/`test:`/`docs:`/`refactor:` + réf. `LOG-NN`) avec commit distinct pour les tests unitaires ; commit de documentation dans la même branche si impact API/statuts ; déplacements de fichiers via `git mv` ; checklist des critères d'acceptation documentée comme pratique manuelle recommandée (non automatisée par `/pr-new`).
- Correction du nom de statut Linear « In code Review » → « In Code Review » (casse réelle vérifiée dans Linear via MCP) dans `commands/pr-new.md`, `commands/README.md` et le skill `workflow-dev-pur` — l'appel `save_issue` de `/pr-new` visait un statut inexistant.
